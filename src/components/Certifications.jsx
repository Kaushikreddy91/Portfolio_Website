import React from 'react';
import { motion } from "framer-motion";
import { CERTIFICATIONS } from "../constants";
import { FaLink } from 'react-icons/fa';
import { CIcon } from '@coreui/icons-react';
import { cibCoursera } from '@coreui/icons';
import { FaAws } from "react-icons/fa";

const Certifications = () => {
    return (
        <div className="border-b border-neutral-900 pb-4">
            <motion.h1
                whileInView={{ opacity: 1, y: 0 }}
                initial={{ opacity: 0, y: -100 }}
                transition={{ duration: 0.5 }}
                className="my-20 text-center text-4xl">Certifications
            </motion.h1>
            <div>
                {CERTIFICATIONS.map((certification, index) => (
                    <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
                        <motion.div
                            whileInView={{ opacity: 1, x: 0 }}
                            initial={{ opacity: 0, x: -100 }}
                            transition={{ duration: 1 }}
                            className="w-full lg:w-1/4 mb-4 lg:mb-0"
                        >
                            <div className="flex items-start gap-4">
                                {certification.iconType === "coreui" && (
                                    <CIcon
                                        icon={cibCoursera}
                                        style={{ width: '28px', height: '28px', fill: '#2A73CC' }}
                                    />
                                )}
                                {certification.iconType === "reacticon" && (
                                    <FaAws className="text-2xl text-yellow-400" />
                                )}
                                <div>
                                    <h6 className="flex items-center font-semibold mb-1">
                                        {certification.role}
                                        {certification.link && (
                                            <a
                                                href={certification.link}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="ml-2 text-purple-400 hover:text-purple-600"
                                            >
                                                <FaLink />
                                            </a>
                                        )}
                                    </h6>
                                    <p className="text-sm text-purple-100">
                                        {certification.company}
                                        <span className="text-neutral-400"> · {certification.year}</span>
                                    </p>
                                </div>
                            </div>
                        </motion.div>
                        <motion.div
                            whileInView={{ opacity: 1, x: 0 }}
                            initial={{ opacity: 0, x: 100 }}
                            transition={{ duration: 1 }}
                            className="w-full max-w-xl lg:w-3/4"
                        >
                            <p className="mb-4 text-neutral-400">{certification.description}</p>
                            {certification.technologies.map((tech, index) => (
                                <span
                                    key={index}
                                    className="mr-2 mt-2 inline-block rounded bg-neutral-900 px-2 py-1 text-sm font-medium text-purple-800"
                                >
                                    {tech}
                                </span>
                            ))}
                        </motion.div>
                    </div>
                ))}
            </div>
        </div>
    )
}
export default Certifications
